...
test("Select a healing character (♥︎) adds health points up to a maximum of 21 health points", () => {
  const player = new Player();
...
test("Select a defending character (♦︎) set defence points according to the cards value", () => {
  const player = new Player();
